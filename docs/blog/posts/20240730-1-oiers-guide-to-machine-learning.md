...
但是到了 8 月中旬（高考后的暑假快要结束了），我发现这本书中几乎全部都是应用，没有我所追求的事物，学习进度就趋于停滞。之后我进行了一个星期的旅游，愈加打断了我的学习。距离开学可能只剩十天，即使还可能有些许的挣扎，我在这一方面的学习可能就要告一段落了。

虽然我机器学习方面的学习走了不少弯路，这个暑假的大部分时间也在迷茫和无助中逝去，但愿这些学习的经历能够化为这流光溢彩的岁月里最绚丽的一抹颜色。
...
不过，如何在本地机运行自己写的机器学习模型也很重要（毕竟你还是想要把结果掌握在自己手中），这就要求你有一台还不错的电脑，并用 `conda` 安装 `python`，`jupyterlab` 和 `torch`。这些操作较为复杂，感兴趣的可以看下面：
...
- [第二章：你的第一个机器学习应用](./20240731-1-first-deep-learning-model.md)
- [第三章：简单的数据伦理学](./20240731-2-data-ethics.md)
- [第四、五章：计算机视觉之图像分类](./20240801-1-image-classification.md)
- [第六、七章：计算机视觉的更多内容](./20240802-1-more-on-computer-vision.md)（超级简略，因为实在没有什么可写的）
- （还是因为学习方向的原因，后面没有坚持了）
